...
import pinCodeStyles from '../../constants/pin-code-style';
...
class CodePin extends Component {
  ...
  render() {
      ...
      pins.push(
        <TextInput
          ...
          style={[
            pinCodeStyles.pin,
            pinStyle,
            this.state.error ? {borderWidth: 4, borderColor: 'red'} : null
          ]}
          returnKeyType={'done'}
    ...
    const error = this.state.error ? (
      <Text style={[pinCodeStyles.error, errorStyle]}>{this.state.error}</Text>
    ) : null;

    return (
      <View style={[pinCodeStyles.container, containerStyle]}>
        <Text style={[pinCodeStyles.text, textStyle]}>{text}</Text>
        <View style={[pinCodeStyles.containerPin, containerPinStyle]}>
          {pins}
        </View>
      </View>
    );
